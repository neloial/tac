...
def download(urls, offset=0):
    ...
    for url in urls[offset:]:
        ...
        Path("/Volumes/Extreme SSD/Temp/TPTraitAuto/data/pdf").mkdir(parents=True, exist_ok=True)
        with open(f"/Volumes/Extreme SSD/Temp/TPTraitAuto/data/pdf/{filename}", 'wb') as f:
...
def check(urls):
    ...
    for url in urls:
        filename = url.split("/")[-1]
        downloads = os.listdir('/Volumes/Extreme SSD/Temp/TPTraitAuto/data/pdf')
